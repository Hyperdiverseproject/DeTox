...
#================================ Filtering contamination from the assembly ===================================================================================================================================================
...
blastp_cline = NcbiblastpCommandline(query=fasta_ORFs_withoutSP, db=args.ToxinDatabase, evalue=args.BlastpToxinesEvalue, outfmt=6,max_target_seqs=1)
stdout, stderr = blastp_cline()
blast_results_withoutSP = pandas.read_csv(StringIO(stdout), sep='\t', header=None, names=["qseqid", "sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore"])
df_ORFs_withoutSP_whithToxinBLast = pandas.merge(dfSequenceWithoutSP,blast_results_withoutSP,left_on="ID",right_on="qseqid")

# ADD blast result of ORF without SP
...
df_global = pandas.merge(df_FinORFs,df_sequenceSignal,left_on="ID",right_on="ID")
df_global = pandas.merge(df_global,df_tHMMER[["target name","query name","accession_Q"]],left_on="ID",right_on="target name")
df_global = pandas.merge(df_global,df_dHMMER[["target name","query name","accession_Q"]],left_on="ID",right_on="target name")
df_global = pandas.merge(df_global,toxDBBlast[["qseqid","sseqid","pident","length","evalue"]],left_on="ID",right_on="qseqid")
...
secreted["Repeats1"] = secreted.apply(lambda x: findRepetition(1,x["AAseqs"]),axis=1)
secreted["Repeats2"] = secreted.apply(lambda x: findRepetition(2,x["AAseqs"]),axis=1)
secreted["Repeats3"] = secreted.apply(lambda x: findRepetition(3,x["AAseqs"]),axis=1)
...
if args.kallisto and args.R1ReadsFile :
    secreted = pandas.read_csv(name+'_secreted_ORFs.alldata',delimiter="\t")
    secreted["ID_transcrit"]=secreted.shortID.str.split(pat=":",n=1,expand=True)[0].str.split(pat="_",n=1,expand=True)[1]

    subprocess.Popen(['kallisto','index','-i','indexKallisto',transcFilesPath]).communicate()
...
if args.SwissProtDatabase:
    ...
    try:
        ...
        secreted = pandas.merge(secreted,blastResult[["qseqid","sseqid","pident","evalue"]],left_on="shortID",right_on="qseqid",how="outer")
        secreted = secreted.drop_duplicates("shortID")
...
"""
CDS-detected.fa = fichier qui contient tous les ORF detecter par ORFfinder
.signalp = fichier qui contient les ORF detecter avec un peptide signal
_sigORFs.fasta = fichier qui filtre ".signalp" pour conserver les signal peptide reconnu seulement
_finalORFs = fichier final contenant les ORFs qui ont un peptide signal mais pas de domaine transmembranaire

HMMER_output1_target_table_final = fichier avec tableau contenant tout les séquence reconnu par HMMER sur la base pfam
HMMER_output2_domain_table_final = fichier avec tableau contenant tout les domaine des séquences reconnu par HMMER sur la base pfam
phobius.out = fichier générer par phobius sur le fichier "_sigORFs.fasta", contient les séquence reconnu avec un domaine transmembranaire

_secreted_ORFs.alldata = fichier final qui contient le résumer de toute les séquences filtrer
"""